...
// HandleEdit updates a review and returns it to moderation.
//
// An edited review must be re-approved. Otherwise the edit path is a way to
// get innocuous text approved and then replace it with something else.
func (s *ReviewServer) HandleEdit(ctx *gin.Context) {
	review, ok := s.authorizeManage(ctx)
	if !ok {
		return
	}
	if review.Status == "withdrawn" {
		ctx.JSON(http.StatusConflict, gin.H{"error": "that review has been withdrawn"})
		return
	}
	...
	if req.Rating != nil {
		if !validRating(*req.Rating) {
			ctx.JSON(http.StatusBadRequest, gin.H{"error": "rating must be 1-5 in half steps"})
			return
		}
	...
	if req.ExpectedGrade != nil {
		if *req.ExpectedGrade != "" && !validExpectedGrade(*req.ExpectedGrade) {
			ctx.JSON(http.StatusBadRequest, gin.H{"error": "expected grade is not valid"})
			return
		}
		patch["expected_grade"] = *req.ExpectedGrade
	}

	if err := s.write.Update("reviews", eq("id", review.ID), patch, nil); err != nil {
		sendInternalError(ctx, "v1/reviews/:id", err)
		return
	}
	...
	ctx.JSON(http.StatusOK, gin.H{
		"status":  "pending",
		"message": "Your edit was saved and the review is awaiting moderation again.",
	})
}
